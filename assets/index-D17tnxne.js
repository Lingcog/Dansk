...
`,this.attachEventListeners(s),this.ensureStyles(),s}getSummaryText(s){const r=(this.currentPhase===1?s.segments.filter(m=>m.type==="gap"):s.phase2_tasks).length,t=Object.values(this.feedback).filter(m=>m==="correct").length;return t===0?"":t===r?`<span class="success">${n("allCorrect")}</span>`:`${t} / ${r} ${n("filled")||"korrekte"}`}renderPhase1(s){const e=[];let r=null;return s.segments.forEach(t=>{t.type==="text"?t.content.split(/(\d+\.\s+)/).forEach(i=>{i.match(/^\d+\.\s+$/)&&(r&&e.push(r),r={segments:[]}),r&&i.trim()&&r.segments.push({type:"text",content:i})}):r&&r.segments.push(t)}),r&&e.push(r),e.map((t,m)=>{let i=null;const l=t.segments.map(a=>{if(a.type==="text"){const o=a.content.match(/\*\*\[(.*?)\]\*\*/);o&&(i=o[1]);let g=a.content.replace(o?o[0]:"","").replace(/\\n/g," ").replace(/\d+\.\s+/g,"").replace(/Dansk:\s*/i,"").replace(/\s+/g," ").trim();return g?`<span class="text-part">${g}</span>`:""}else{const o=this.feedback[a.id]==="correct",g=this.feedback[a.id]==="wrong";return`
            ...
            @media (max-width: 600px) {
                ...
                .sent-clue { font-size: 0.95rem !important; margin-bottom: 0.6rem !important; }
