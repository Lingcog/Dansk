...
`,this.attachEventListeners(d),this.ensureStyles(),d}getSummaryText(d){const a=(this.currentPhase===1?d.segments.filter(g=>g.type==="gap"):d.phase2_tasks).length,n=Object.values(this.feedback).filter(g=>g==="correct").length;return n===0?"":n===a?`<span class="success">${t("allCorrect")}</span>`:`${n} / ${a} ${t("filled")||"korrekte"}`}renderPhase1(d){return d.segments.map(e=>{if(e.type==="text")return`<span class="text-part">${e.content.replace(/\\n/g,"<br>")}</span>`;if(e.type==="gap"){const a=this.feedback[e.id]==="correct",n=this.feedback[e.id]==="wrong";return`
    ...
    `;const b=h=>h==="Et"?"sødt":h==="Det"||h==="Mit"?"søde":"sødt";u.textContent=b(a);function k(h){let f="1.5s",T=3,E="#ffeb3b",D=2;h==="hurtigt"?(f="0.4s",E="#4caf50",T=5):h==="langsomt"?(f="3.5s",E="#2196f3",T=2):h==="meget"?(f="1s",E="#f44336",T=8,D=4):h==="lidt"&&(f="2s",E="#9e9e9e",T=1,D=1);let x="";for(let y=0;y<T;y++){const v=30+(y-(T-1)/2)*10;x+=`
        ...
        `,document.head.appendChild(h)}}function Le(m,d,e={}){const a=e.nounAnchor||A.nounAnchor||"Et",n=e.selectedNoun||A.selectedNoun||"barn",g=e.selectedVerb||A.selectedVerb||"spiser",s=e.selectedAdverb||A.selectedAdverb||"langsomt",o=e.adjectivePart||"sødt";let l="fordi";m.innerHTML=`
        <div class="grounding-container animate-in">
            <div class="header-section">
